...
Faites attention de bien ajouter le préfixe `mock` à la variable `mockPostList`. Sinon, vous obtiendrez l'erreur&nbsp;: "The module factory of `jest.mock()` is not allowed to reference any out-of-scope variables.". C'est spécifique à `jest` et vous pouvez en savoir plus sur ce comportement dans leur documentation.
...
Il existe plusieurs façons de définir des `mocks` dans Jest. Celui utilisé dans l'exemple ci-dessus est le plus simple. Pour des alternatives plus puissantes, vous pouvez consulter axios-mock-adapter ou msw, entre autres.
:::

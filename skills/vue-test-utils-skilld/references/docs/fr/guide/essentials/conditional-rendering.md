...
Cet article est également disponible sous la forme d'une courte vidéo.
...
`wrapper` a une méthode `get()` qui recherche un élément existant. Il utilise la syntaxe de `querySelector`.
...
Parfois, vous ne voulez que masquer/afficher un élément tout en le conservant dans le DOM. Vue propose `v-show` pour des scénarios de ce genre. (Vous retrouverez les différences entre `v-if` et `v-show` ici).
...
::: warning
`isVisible()` ne fonctionne correctement que si le wrapper est attaché au DOM via la `attachTo`. Ceci est d'autant plus important lors de l'utilisation de **happy-dom** comme environnement de test. Sans `attachTo`, la visibilité dirigée par le CSS (tel que `v-show`) peut ne pas être détectée correctement. Pour plus de détails, consulter la documentation d'API de `isVisible()`.
:::

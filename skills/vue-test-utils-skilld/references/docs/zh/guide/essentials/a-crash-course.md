...
我们首先导入 `mount`——这是在 VTU 中渲染组件的主要方式。使用 `test` 函数声明一个测试，并给出简短的描述。`test` 和 `expect` 函数在大多数测试运行器中是全局可用的 (此示例使用 Jest)。如果 `test` 和 `expect` 看起来令人困惑，Jest 文档中有一个更简单的示例，可以帮助你理解它们的用法和工作原理。

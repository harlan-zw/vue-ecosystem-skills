...
The `button` input uses HTML's native button element. The `label` prop is used to populate the text of the button — or alternatively you can use child content.
...
You can also bind event listeners. In React JSX this means normal camelCased props like <code>onClick</code>:

### Example: Button input

#### button-events.react.jsx

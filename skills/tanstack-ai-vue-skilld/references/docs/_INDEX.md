...
- [Code Mode Isolate Drivers](./guides/code-mode-isolates.md): Isolate drivers provide the secure sandbox runtimes that Code Mode uses to execute generated TypeScript. All drivers implement the same IsolateDriv...
- [Code Mode with Skills](./guides/code-mode-with-skills.md): Skills extend Code Mode with a persistent library of reusable TypeScript snippets. When the LLM writes a useful piece of code — say, a function tha...
- [Code Mode](./guides/code-mode.md): Code Mode lets an LLM write and execute TypeScript programs inside a secure sandbox. Instead of making one tool call at a time, the model writes a ...
